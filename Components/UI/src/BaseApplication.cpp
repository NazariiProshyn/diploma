...
BaseApplication::BaseApplication() : _hInstance{GetModuleHandle(NULL)}
{
    ...
    std::cout << "run compiler" << std::endl;
    if (compiler.checkFile() && compiler.lexical())
    {
        std::cout << "lexical checks passed" << std::endl;
        if (parcer.checkFile())
        {
            ...
            else
            {
                ...
                _mainWindow.addWidget<Label>("Error", Size{250, 350}, Position{15, 15}, Text{parcer.getError()});
            }
        }
        else
        {
    ...
    else
    {
        // Error window
        _mainWindow = Window("Error Information", Size(300, 400), _hInstance);
        _mainWindow.addWidget<Label>("Error", Size{250, 350}, Position{15, 15}, Text{compiler.getError()});
    }
...
{
    _mainWindow.addWidget<Button>(params.getName(), Size{params.getSizeVert(), params.getSizeGorz()}, Position{params.getPositionGorz(), params.getPositionVert()}, Text{params.text.getText(), Font(params.text.getHeight(), params.text.getBold(), params.text.getItalic(), params.text.getUnderline(), false, params.text.getTitle())}, params.getclickable(), params.getDoubleclickable());
}
...
{
    _mainWindow.addWidget<CheckBox>(params.getName(), Size{params.getSizeVert(), params.getSizeGorz()}, Position{params.getPositionGorz(), params.getPositionVert()}, Text{params.text.getText(), Font(params.text.getHeight(), params.text.getBold(), params.text.getItalic(), params.text.getUnderline(), false, params.text.getTitle())}, params.getclickable());
}
